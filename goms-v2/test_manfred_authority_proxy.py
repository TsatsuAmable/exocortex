...
class ManfredAuthorityProxyTests(unittest.TestCase):
    def setUp(self):
        ...
        self.key = b"authority-test-key"
        self.server = Server(("127.0.0.1", 0), Handler, root=self.root,
                             allowed_client="127.0.0.1", key=self.key)
    ...
    def tearDown(self):
        ...
        self.tmp.cleanup()
    def signed(self, body, request_id="req-1", timestamp=None, key=None):
        timestamp = str(timestamp or int(time.time()))
        raw = json.dumps(body, separators=(",", ":"), sort_keys=True).encode()
        path = "/v1/manfred/intent-command"
        canonical = f"POST\n{path}\n{timestamp}\n{request_id}\n".encode() + raw
        signature = hmac.new(key or self.key, canonical, hashlib.sha256).hexdigest()
    ...
    def post(self, body, **kwargs):
        ...
        req = urllib.request.Request(self.base + "/v1/manfred/intent-command",
    ...
    def test_missing_bad_stale_and_replayed_signatures_are_rejected(self):
        ...
        status, body = self.post({"type":"approve_intent"}, request_id="bad-key", key=b"wrong")
        self.assertEqual(status, 401); self.assertEqual(body["error"], "bad_signature")

        stale = int(time.time()) - 1000
        status, body = self.post({"type":"approve_intent"}, request_id="stale", timestamp=stale)
        self.assertEqual(status, 401); self.assertEqual(body["error"], "stale_request")

        body = {"type":"approve_intent","target_id":"missing","idempotency_key":"replay",
                "payload":{"resolved_by":"human:test"}}
        first_status, _ = self.post(body, request_id="same-request")
        second_status, second = self.post(body, request_id="same-request")
        self.assertNotEqual(first_status, 401)
        self.assertEqual(second_status, 401); self.assertEqual(second["error"], "replay")

    def test_signed_unknown_action_is_rejected(self):
        status, body = self.post({"type":"shell","target_id":"x","idempotency_key":"bad",
                                  "payload":{}}, request_id="unknown-action")
    ...
    def test_valid_signed_intent_command_executes_shared_control_path(self):
        ...
        command = {"type":"approve_intent","target_id":intent_id,
        ...
        with self.store.connect() as con:
            ...
            self.assertEqual(con.execute("SELECT count(*) FROM manfred_commands WHERE idempotency_key='proxy-approve-1'").fetchone()[0],1)
