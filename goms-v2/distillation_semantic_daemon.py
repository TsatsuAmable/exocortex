...
STAGE_ORDER = ('validate','reconcile','gate','adjudicate','shape','promote','surface')
SCRIPTS = {
    ...
    'shape': 'distillation_graphshape_review.py','promote': 'distillation_promote.py',
    'surface': 'distillation_review_surface.py',
}
...
def stage_counts(db_path=DB):
    with closing(sqlite3.connect(db_path)) as c:
        ...
        q['shape_missing'] = c.execute("select count(*) from distillation_promotion_gate g left join distillation_graphshape_reviews s on s.candidate_id=g.candidate_id where g.decision='AUTO_READY' and g.gate_fingerprint is not null and s.candidate_id is null").fetchone()[0]
        q['promotable'] = c.execute("select count(*) from distillation_reconciliation_proposals p join distillation_promotion_gate g on g.candidate_id=p.candidate_id join distillation_graphshape_reviews s on s.candidate_id=p.candidate_id where p.status='candidate' and g.decision='AUTO_READY' and g.gate_fingerprint is not null and s.verdict='ACCEPT'").fetchone()[0]
        q['surface_dirty'] = 1 if review_surface_dirty(c) else 0
        return q


def choose_stage(counts, cursor=0, promotion_enabled=True):
    keys={'validate':'unvalidated','reconcile':'unreconciled','gate':'gate_missing','adjudicate':'review_actionable','shape':'shape_missing','promote':'promotable','surface':'surface_dirty'}
    start=int(cursor) % len(STAGE_ORDER)
    for offset in range(len(STAGE_ORDER)):
        stage=STAGE_ORDER[(start+offset) % len(STAGE_ORDER)]
        if stage=='promote' and not promotion_enabled: continue
        if int(counts.get(keys[stage],0)) > 0: return stage
    return None
...
def run_forever(active_sleep=1.0, idle_sleep=15.0):
    with LOCK.open('w') as lock:
        ...
        while True:
            counts = stage_counts(); stage = choose_stage(counts,cursor=cursor,promotion_enabled=promotion_enabled)
