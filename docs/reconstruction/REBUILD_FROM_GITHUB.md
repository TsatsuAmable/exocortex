...
Before starting Hermes, apply the ordered Exocortex-qualified compatibility patch set. It makes
main-provider fallback context-safe and enforces the opted-in Aineko control-turn budget: a smaller
fallback is skipped when the live request cannot fit its safe input window, Ollama runtime context is
re-resolved when the fallback model changes, and exact Status/What next?/Proceed turns receive a
one-shot policy note plus a three-iteration ceiling.
...
Do not force these patches onto another Hermes revision. The applicator applies them in declared
order and fails closed when the checkout is not at the qualified base commit or patch order drifts.
